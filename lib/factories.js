...
const createSanity = async ({execa, repo, orgname, spinnies}) => {
  ...
  await execa.sync('git', ['init'], {cwd});
  await execa.sync('git', ['add', '.'], {cwd});
  await execa.sync('git', ['commit', '-m', '"Initial Sanity commit"'], {cwd});
  await execa.sync('git', ['remote', 'add', 'origin', url], {cwd});
  await execa.sync('git', ['push', '-u', 'origin', 'master'], {cwd});
