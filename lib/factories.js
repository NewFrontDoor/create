...
const {buildOutSanity} = require('./sanity-fns.js');
const fs = require('fs');
...
const createSanity = async ({execa, orgname, spinnies, wantsUI, palette}) => {
  ...
  await buildOutSanity({cwd: process.cwd() + '/sanity/', palette});

  if (wantsUI) {
    await execa('yarn', ['add', ...dependencies], {
      cwd: process.cwd() + '/sanity'
    }).catch((error) => {
      throw error;
    });
    const sanityJson = JSON.parse(
      fs.readFileSync(process.cwd() + '/sanity/sanity.json', (err) => {
        if (err) throw err;
      })
    );
    sanityJson.plugins = sanityJson.plugins.concat(dependencies);
    sanityJson.parts = sanityJson.parts.concat(parts);
    fs.writeFileSync('sanity/sanity.json', JSON.stringify(sanityJson, null, 2));
  }
